...
class TD3Agent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch, noise_scale: float, update_actor: bool = True):
        ...
        def critic_loss_fn(critic_params):
            ...
            next_actions_noise = (
                jax.random.normal(next_key, next_dist.loc.shape) * noise_scale
            )
            next_actions_noise = jnp.clip(
                next_actions_noise,
                -agent.config["td3_noise_clip"],
                agent.config["td3_noise_clip"],
            )
            next_actions = next_dist.loc + next_actions_noise
            next_actions = jnp.clip(next_actions, -1, 1)

            next_qs = agent.target_critic(
    ...
    def update_high_utd(agent, batch, noise_scale):
        ...
        assert (
            batch_size % agent.config["utd"] == 0
        ), f"Batch {batch_size} must divide by UTD {agent.config['utd']}"
        assert (
            batch_size % agent.config["actor_utd"] == 0
        ), f"Batch {batch_size} must divide by Actor UTD {agent.config['actor_utd']}"
        ...
        def scan_body(carry, data):
            ...
            agent, info = agent.update(
                minibatch, noise_scale=noise_scale, update_actor=False
            )
            return (agent,), info

        def make_minibatch(data: jnp.ndarray):
            return jnp.reshape(
                data, (agent.config["utd"], minibatch_size) + data.shape[1:]
            )
        ...
        def scan_body(carry, data):
            ...
            (minibatch,) = data
            agent, info = agent.update(
                minibatch, noise_scale=noise_scale, update_actor=True
            )
            return (agent,), info
        ...
        def make_minibatch(data: jnp.ndarray):
            return jnp.reshape(
                data,
                (agent.config["actor_utd"], minibatch_size) + data.shape[1:],
            )
...
def create_agent(seed: int, observations: jnp.ndarray, actions: jnp.ndarray, **kwargs):
    ...
    actor = TrainState.create(
        actor_def, actor_params, tx=optax.adam(learning_rate=config["actor_lr"])
    )
...
def main(_):
    ...
    for i in tqdm.tqdm(
        ...
        if i < FLAGS.start_steps:
            ...
            action, entropy = (
                agent.sample_actions(obs, seed=key, temperature=noise_scale),
                0.0,
            )
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            eval_info, trajectories = evaluate(
                policy_fn,
                eval_env,
            ...
            for t in trajectories:
                ...
                for r in reversed(range(len(rewards) - 1)):
                    discounted_mc_returns[r] = (
                        rewards[r]
                        + FLAGS.agent.discount * masks[r] * discounted_mc_returns[r + 1]
                    )
