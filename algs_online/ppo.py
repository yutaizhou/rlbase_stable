...
from utils.networks import Critic, Policy, ValueCritic, ensemblize
from utils.rms import RunningMeanStd
from utils.train_state import TrainState, supply_rng, target_update
from utils.wandb import default_wandb_config, get_flag_dict, setup_wandb
...
)  # Multiplied by num_envs. Batch will be [batch_size, num_envs, ...]
flags.DEFINE_integer("num_envs", 16, "Number of environments to run in parallel.")
flags.DEFINE_integer(
    "num_steps_per_update", 128, "Number of steps per epoch."
)  # Total-Batch = num_envs * num_steps_per_update
...
class PPOAgent(flax.struct.PyTreeNode):
    ...
    def compute_gae(agent, batch):
        ...
        def scan_fn(lastgaelam, inputs):
            ...
            advantage = (
                delta + mask * agent.config["gamma"] * agent.config["lam"] * lastgaelam
            )
            return advantage, advantage

        zeros = jnp.zeros((FLAGS.num_envs,))
        _, batch_advantages = jax.lax.scan(
            scan_fn,
            zeros,
            (batch["rewards"], batch["masks"], batch_value, batch_next_value),
            reverse=True,
        )
        batch_normalized_adv = (batch_advantages - jnp.mean(batch_advantages)) / (
            jnp.std(batch_advantages) + 1e-7
        )
        batch_values_gae = batch_value + batch_advantages
        assert batch_advantages.shape == batch_value.shape, (
    ...
    def update(agent, batch):
        ...
        def critic_loss_fn(critic_params):
            ...
            if values.shape != batch_values_gae.shape:
                raise ValueError(
                    f"Values shape {values.shape} does not match batch_values shape {batch_values_gae.shape}"
                )
        ...
        def actor_loss_fn(actor_params):
            ...
            assert batch_normalized_adv.shape == ratio.shape, (
                batch_normalized_adv.shape,
                ratio.shape,
            )

            if agent.config["use_clipping"]:
                pg_loss1 = -batch_normalized_adv * ratio
                pg_loss2 = -batch_normalized_adv * jnp.clip(
                    ratio,
            ...
            actions_live, log_probs_live = action_dist.sample_and_log_prob(
                seed=curr_key
            )
    ...
    @jax.jit
    def sample_actions(
        agent, observations: np.ndarray, *, seed: Any, temperature: float = 1.0
    ) -> jnp.ndarray:
        if type(observations) is dict:
            observations = jnp.concatenate(
                [observations["observation"], observations["goal"]], axis=-1
            )
...
def create_agent(seed: int, observations: jnp.ndarray, actions: jnp.ndarray, **kwargs):
    ...
    actor_def = Policy(
        ...
        tanh_squash_distribution=config["use_tanh"],
    ...
    actor_tx = optax.chain(
        ...
        optax.adam(learning_rate=config["actor_lr"]),
    )
    ...
    return PPOAgent(
        rng, critic=critic, actor=actor, rms_obs=rms_obs, config=config_dict
    )
...
def main(_):
    ...
    for i in tqdm.tqdm(
        ...
        action, log_probs = agent.sample_actions_and_logprobs(
            obs, seed=key
        )  # Range [-1, 1].
        ...
        if done[0]:
            exploration_metrics = {
                f"exploration/{k}": v for k, v in flatten(info[0]).items()
            }
        ...
        if i % FLAGS.num_steps_per_update == 0:
            ...
            if FLAGS.agent.normalize_input:
                ...
                batch["next_observations"] = agent.rms_obs.norm(
