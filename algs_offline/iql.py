...
config_flags.DEFINE_config_dict(
    "gcdataset", GCDataset.get_default_config(), lock_config=False
)
        ...
        def actor_loss_fn(actor_params):
            if agent.config["actor_loss_type"] == "awr":
                ...
                if agent.config["target_extraction"]:
                    qs = agent.target_critic(
                        batch["observations_policy"], batch["actions"]
                    )
                else:
...
def create_agent(seed: int, observations: jnp.ndarray, actions: jnp.ndarray, **kwargs):
    ...
    mlp_kwargs = dict(
        activation=config["activation"], use_layer_norm=config["use_layer_norm"]
    )
    ...
    critic_def = ensemblize(Critic, num_qs=config["num_qs"])(
        config["hidden_dims"], mlp_kwargs=mlp_kwargs
    )
...
def main(_):
    ...
    if FLAGS.agent.goal_conditioned:
        ...
        example_obs = np.concatenate(
            [example_batch["observations"], example_batch["goals"]], axis=-1
        )
    ...
    for i in tqdm.tqdm(
        ...
        if FLAGS.agent.goal_conditioned:
            batch["observations_policy"] = np.concatenate(
                [batch["observations"], batch["policy_goals"]], axis=-1
            )
            batch["observations"] = np.concatenate(
                [batch["observations"], batch["goals"]], axis=-1
            )
            batch["next_observations"] = np.concatenate(
                [batch["next_observations"], batch["goals"]], axis=-1
            )
        else:
        ...
        if i % FLAGS.log_interval == 0:
            ...
            if FLAGS.use_validation:
                ...
                valid_metrics = {
                    f"validation/{k}": v for k, v in valid_update_info.items()
                }
                wandb.log(valid_metrics, step=i)

                wandb.log(
                    {
                        "training/actor_valid_difference": (
                            valid_update_info["actor_loss"] - update_info["actor_loss"]
                        )
                    },
                    step=i,
                )
                wandb.log(
                    {
                        "training/critic_valid_difference": (
                            valid_update_info["critic_loss"]
        ...
        if i % FLAGS.eval_interval == 0 or i == 1:
            ...
            try:
                eval_metrics["evaluation/episode.return.normalized"] = (
                    eval_env.get_normalized_score(eval_info["episode.return"])
                )
                print(
                    f'evaluation/episode.return.normalized: {eval_metrics["evaluation/episode.return.normalized"]}'
                )
            except:
                pass
            if record_video:
            ...
            if "antmaze-large" in FLAGS.env_name or "maze2d-large" in FLAGS.env_name:
                ...
                if (
                    "antmaze-large" in FLAGS.env_name
                    or "maze2d-large" in FLAGS.env_name
                ):
                    ...
                    pred_value_img = d4rl_ant.value_image(
                        eval_env, dataset, get_gcvalue
                    )
                ...
                if "maze2d-large" in FLAGS.env_name:
                    ...
                    traj_actions = np.concatenate(
                        [t["action"] for t in trajs], axis=0
                    )  # (T, A)
