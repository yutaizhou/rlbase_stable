...
class BCAgent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch):
        ...
        new_target_actor = target_update(
            agent.actor, agent.target_actor, agent.config["actor_tau"]
        )
    ...
    def sample_actions(
        ...
        if agent.config["eval_target_actor"]:
            actions = agent.target_actor(observations, temperature=temperature).sample(
                seed=seed
            )
        else:
            actions = agent.actor(observations, temperature=temperature).sample(
                seed=seed
            )
...
def create_agent(seed: int, observations: jnp.ndarray, actions: jnp.ndarray, **kwargs):
    ...
    actor_def = Policy(
        ...
        mlp_kwargs=dict(use_layer_norm=False, activations=nn.relu),
    )
