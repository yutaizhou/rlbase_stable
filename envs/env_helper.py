...
def make_env(env_name, **kwargs):
    if "exorl" in env_name:
        ...
        frame_skip = kwargs["frame_skip"] if "frame_skip" in kwargs else 1
        visualize_reward = (
            kwargs["visualize_reward"] if "visualize_reward" in kwargs else False
        )
        env = dmc2gym.make(
            domain_name=suite,
            task_name=task,
        ...
        "halfcheetah-" in env_name or "walker2d-" in env_name or "hopper-" in env_name
    ):  # D4RL Mujoco
        import d4rl
...
def get_dataset(env, env_name, **kwargs):
    if "exorl" in env_name:
        ...
        "ant" in env_name
        or "maze2d" in env_name
        or "kitchen" in env_name
        or "halfcheetah" in env_name
