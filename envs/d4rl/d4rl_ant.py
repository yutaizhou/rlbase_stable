...
class MazeWrapper(gym.Wrapper):
    ...
    def XY(self, n=20, m=30):
        ...
        X = np.linspace(
            bl[0] + 0.04 * (tr[0] - bl[0]), tr[0] - 0.04 * (tr[0] - bl[0]), m
        )
        Y = np.linspace(
...
class GoalReachingMaze(MazeWrapper):
    def __init__(self, env_name, discrete_xy=False):
...
def discretize_obs(ob, num_bins=32, disc_type="tanh", disc_temperature=1.0):
    ...
    disc_dims = 2
    bins = np.linspace(
        min_ob, max_ob, num_bins
    ).T  # [num_bins,] values from min_ob to max_ob
    ...
    if disc_type == "twohot":
        ...
        bin_diff_normalized = (
            bin_diff / np.expand_dims(bin_size[:disc_dims], -1) * disc_temperature
        )
...
def plot_value(goal_observation, env, dataset, value_fn, ax):
    ...
    if "antmaze" in env.env_name:
        ...
        if env.discrete_xy:
            xy_observations = discretize_obs(xy_observations)
            assert xy_observations.shape[1] == 91
        else:
            assert xy_observations.shape[1] == 29
    elif "maze2d" in env.env_name:
