...
class GCDataset:
    ...
    def __post_init__(self):
        ...
        self.terminal_locs = np.concatenate(
            [self.terminal_locs, [self.dataset.size - 1]]
        )
    ...
    def sample(self, batch_size: int, indx=None):
        ...
        distance = np.random.rand(batch_size)
        policy_traj_goal_indx = np.round(
            (
                np.minimum(indx + 1, final_state_indx) * distance
                + final_state_indx * (1 - distance)
            )
        ).astype(int)
        batch["policy_goals"] = jax.tree_map(
            lambda arr: arr[policy_traj_goal_indx], self.dataset["observations"]
        )
    ...
    def train_valid_split(self, ratio):
        ...
        assert np.equal(
            diff, np.ones_like(diff) * traj_len
        ).all()  # make sure the trajectories are of the same length.

        reshaped_dataset = jax.tree_map(
            lambda arr: np.reshape(arr, (-1, traj_len, *arr.shape[1:])),
            self.dataset._dict,
        )
        shuffled_indicies = np.random.permutation(
            reshaped_dataset["observations"].shape[0]
        )
        ...
        dataset_train = jax.tree_map(
            lambda arr: np.reshape(arr, (-1, *arr.shape[2:])), dataset_train
        )
        dataset_valid = jax.tree_map(
            lambda arr: np.reshape(arr, (-1, *arr.shape[2:])), dataset_valid
        )
        ...
        return GCDataset(dataset_train, **dataset_config), GCDataset(
            dataset_valid, **dataset_config
        )
